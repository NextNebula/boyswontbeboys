<template>
  <div class="agenda">
    <table>
      <tr class="month-header">
        <td colspan="4">AUGUSTUS 2019</td>
      </tr>
      <tr>
        <td>do 8</td>
        <td>Den Bosch</td>
        <td>Boulevard Festival</td>
        <td><a href="https://www.festivalboulevard.nl/nl/programma/id-4323/boys-won't-be-boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>vr 9</td>
        <td>Den Bosch</td>
        <td>Boulevard Festival</td>
        <td><a href="https://www.festivalboulevard.nl/nl/programma/id-4323/boys-won't-be-boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>za 10</td>
        <td>Den Bosch</td>
        <td>Boulevard Festival</td>
        <td><a href="https://www.festivalboulevard.nl/nl/programma/id-4323/boys-won't-be-boys/">Tickets</a></td>
      </tr>
      <tr class="month-header">
        <td colspan="4">SEPTEMBER 2019</td>
      </tr>
      <tr>
        <td>do 12</td>
        <td>Tilburg</td>
        <td>De Nieuwe Vorst</td>
        <td><a href="https://denieuwevorst.nl/programma/boys-wont-be-boys">Tickets</a></td>
      </tr>
      <tr>
        <td>vr 13</td>
        <td>Tilburg</td>
        <td>De Nieuwe Vorst</td>
        <td><a href="https://denieuwevorst.nl/programma/boys-wont-be-boys">Tickets</a></td>
      </tr>
      <tr>
        <td>za 14</td>
        <td>Tilburg</td>
        <td>De Nieuwe Vorst</td>
        <td><a href="https://denieuwevorst.nl/programma/boys-wont-be-boys">Tickets</a></td>
      </tr>
      <tr>
        <td>do 19</td>
        <td>Utrecht</td>
        <td>Stadsschouwburg</td>
        <td><a href="https://stadsschouwburg-utrecht.nl/agenda/2019-20/boys-wont-be-boys/">Tickets</a></td>
      </tr>
      <tr class="month-header">
        <td colspan="4">OKTOBER 2019</td>
      </tr>
      <tr>
        <td>za 5</td>
        <td>Rotterdam</td>
        <td>Walhalla</td>
        <td><a href="https://www.theaterwalhalla.nl/voorstelling/boys-wont-be-boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>wo 9</td>
        <td>Enschede</td>
        <td>Wilminktheater</td>
        <td><a href="https://www.wilminktheater.nl/programma/!/4987/zou-jij-het-zijn-o.l.v.-rikkert-van-huisstede/boys-wont-be-boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>vr 11</td>
        <td>Almere</td>
        <td>KAF</td>
        <td><a href="https://kaf.nl/podium/agenda/seizoen-2019-2020/boys-wont-be-boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>di 15</td>
        <td>Amsterdam</td>
        <td>Theater Bellevue</td>
        <td><a href="https://www.theaterbellevue.nl/agenda/2224/Rikkert_van_Huisstede/Boys_won_039_t_be_boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>wo 16</td>
        <td>Amsterdam</td>
        <td>Theater Bellevue</td>
        <td><a href="https://www.theaterbellevue.nl/agenda/2224/Rikkert_van_Huisstede/Boys_won_039_t_be_boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>do 17</td>
        <td>Amsterdam</td>
        <td>Theater Bellevue</td>
        <td><a href="https://www.theaterbellevue.nl/agenda/2224/Rikkert_van_Huisstede/Boys_won_039_t_be_boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>vr 18</td>
        <td>Amsterdam</td>
        <td>Theater Bellevue</td>
        <td><a href="https://www.theaterbellevue.nl/agenda/2224/Rikkert_van_Huisstede/Boys_won_039_t_be_boys/">Tickets</a></td>
      </tr>
      <tr>
        <td>za 19</td>
        <td>Amsterdam</td>
        <td>Theater Bellevue</td>
        <td><a href="https://www.theaterbellevue.nl/agenda/2224/Rikkert_van_Huisstede/Boys_won_039_t_be_boys/">Tickets</a></td>
      </tr>
      <tr class="month-header">
        <td colspan="4">NOVEMBER 2019</td>
      </tr>
      <tr>
        <td>do 14</td>
        <td>Arnhem</td>
        <td>Theater aan de Rijn</td>
        <td><a href="https://www.theateraanderijn.nl/programma/boys-wont-be-boys-olv-rikkert-van-huisstede">Tickets</a></td>
      </tr>
      <tr>
        <td>n.n.b.</td>
        <td>Groningen</td>
        <td>Jonge Harten festival</td>
        <td></td>
      </tr>
      <tr class="month-header">
        <td colspan="4">DECEMBER 2019</td>
      </tr>
      <tr>
        <td>di 3</td>
        <td>Den Haag</td>
        <td>Zaal 3</td>
        <td><a href="https://zaal3.nl/agenda/boys-wont-be-boys/">Tickets</a></td>
      </tr>
      <tr class="month-header">
        <td colspan="4">JANUARI 2020</td>
      </tr>
      <tr>
        <td>vr 10</td>
        <td>Breda</td>
        <td>Chassé theater</td>
        <td><a href="https://www.chasse.nl/theater/programma/boys-won-t-be-boys">Tickets</a></td>
      </tr>
    </table>
  </div>
</template>

<style>
.agenda div:last-of-type {
  margin-bottom: 0;
}

.month-header td {
  padding-top: 1rem;
}

.month-header {
  font-weight: 600;
  text-align: center;
}

table {
  text-align: left;
  margin-top: -1rem;
}

td {
  padding-left: 0.5rem;
  padding-right: 0.5rem;
}
</style>
